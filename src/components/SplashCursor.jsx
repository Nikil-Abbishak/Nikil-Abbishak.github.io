...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function updateKeywords() {
      let kw = []; if (config.SHADING) kw.push('SHADING');
    ...
    function calcDeltaTime() {
      let now = Date.now(), dt = (now - lastUpdateTime) / 1000;
    ...
    function updatePointerDownData(ptr, id, px, py) {
      ptr.id = id; ptr.down = true; ptr.moved = false;
      ptr.texcoordX = px/canvas.width; ptr.texcoordY = 1.0 - py/canvas.height;
      ptr.prevTexcoordX = ptr.texcoordX; ptr.prevTexcoordY = ptr.texcoordY;
      ptr.deltaX = 0; ptr.deltaY = 0; ptr.color = generateColor();
    }

    function updatePointerMoveData(ptr, px, py, color) {
      ptr.prevTexcoordX = ptr.texcoordX; ptr.prevTexcoordY = ptr.texcoordY;
      ptr.texcoordX = px/canvas.width; ptr.texcoordY = 1.0 - py/canvas.height;
    ...
    function onMouseMove(e) {
      let ptr = pointers[0], px = scaleByPixelRatio(e.clientX), py = scaleByPixelRatio(e.clientY);
    ...
    function onTouchStart(e) {
      const touches = e.targetTouches; let ptr = pointers[0];
      for (let i = 0; i < touches.length; i++)
        updatePointerDownData(ptr, touches[i].identifier, scaleByPixelRatio(touches[i].clientX), scaleByPixelRatio(touches[i].clientY));
    }
    function onTouchMove(e) {
      const touches = e.targetTouches; let ptr = pointers[0];
      for (let i = 0; i < touches.length; i++)
        updatePointerMoveData(ptr, scaleByPixelRatio(touches[i].clientX), scaleByPixelRatio(touches[i].clientY), ptr.color);
    }
    function onTouchEnd(e) {
      const touches = e.changedTouches; let ptr = pointers[0];
